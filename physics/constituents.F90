...
use physcons,        only: shr_const_rgas=>con_rgas
...
integer, parameter, public :: pcnst  = 5      ! number of advected constituents (including water vapor)
...
real(kind_phys),    public :: cnst_cp  (pcnst)          ! specific heat at constant pressure (J/kg/K)
real(kind_phys),    public :: cnst_cv  (pcnst)          ! specific heat at constant volume (J/kg/K)
real(kind_phys),    public :: cnst_mw  (pcnst)          ! molecular weight (kg/kmole)
...
real(kind_phys),    public :: cnst_rgas(pcnst)          ! gas constant ()
real(kind_phys),    public :: qmin     (pcnst)          ! minimum permitted constituent concentration (kg/kg)
real(kind_phys),    public :: qmincg   (pcnst)          ! for backward compatibility only
...
subroutine cnst_add (name, mwc, cpc, qminc, &
   ...
   real(kind_phys),intent(in)    :: mwc    ! constituent molecular weight (kg/kmol)
   real(kind_phys),intent(in)    :: cpc    ! constituent specific heat at constant pressure (J/kg/K)
   real(kind_phys),intent(in)    :: qminc  ! minimum value of mass mixing ratio (kg/kg)
