...
!
! Init:
! call wv_sat_methods_init(kind_phys, <constants>, errstring)
!
...
real(kind_phys) :: tmelt   ! Melting point of water at 1 atm (K)
real(kind_phys) :: h2otrip ! Triple point temperature of water (K)
real(kind_phys) :: tboil   ! Boiling point of water at 1 atm (K)

real(kind_phys) :: ttrice  ! Ice-water transition range

real(kind_phys) :: epsilo  ! Ice-water transition range
real(kind_phys) :: omeps   ! 1._kind_phys - epsilo
...
subroutine wv_sat_methods_init(kind, tmelt_in, h2otrip_in, tboil_in, &
  ...
  real(kind_phys), intent(in) :: h2otrip_in
  real(kind_phys), intent(in) :: tboil_in
  real(kind_phys), intent(in) :: ttrice_in
  real(kind_phys), intent(in) :: epsilo_in
  character(len=*), intent(out)  :: errstring

  errstring = ' '

  if (kind /= kind_phys) then
     write(errstring,*) 'wv_sat_methods_init: ERROR: ', &
          kind,' was input kind but ',kind_phys,' is internal kind.'
     return
  end if

  if (ttrice_in < 0._kind_phys) then
     write(errstring,*) 'wv_sat_methods_init: ERROR: ', &
...
elemental function wv_sat_svp_to_qsat(es, p) result(qs)
  ...
  real(kind_phys), intent(in) :: es  ! SVP
  real(kind_phys), intent(in) :: p   ! Current pressure.
  real(kind_phys) :: qs

  ! If pressure is less than SVP, set qs to maximum of 1.
  if ( (p - es) <= 0._kind_phys ) then
     qs = 1.0_kind_phys
  else
...
elemental function wv_sat_svp_trans(t, idx) result (es)
  ...
  real(kind_phys) :: esice      ! Saturation vapor pressure over ice
  real(kind_phys) :: weight     ! Intermediate scratch variable for es transition
...
!
  if (t < tmelt) then
     ...
     es = weight*esice + (1.0_kind_phys - weight)*es
  end if
...
elemental function GoffGratch_svp_water(t) result(es)
  real(kind_phys), intent(in) :: t  ! Temperature in Kelvin
  real(kind_phys) :: es             ! SVP in Pa

  ! uncertain below -70 C
  es = 10._kind_phys**(-7.90298_kind_phys*(tboil/t-1._kind_phys)+ &
       5.02808_kind_phys*log10(tboil/t)- &
       1.3816e-7_kind_phys*(10._kind_phys**(11.344_kind_phys*(1._kind_phys-t/tboil))-1._kind_phys)+ &
       8.1328e-3_kind_phys*(10._kind_phys**(-3.49149_kind_phys*(tboil/t-1._kind_phys))-1._kind_phys)+ &
       log10(1013.246_kind_phys))*100._kind_phys
...
elemental function GoffGratch_svp_ice(t) result(es)
  ...
  ! good down to -100 C
  es = 10._kind_phys**(-9.09718_kind_phys*(h2otrip/t-1._kind_phys)-3.56654_kind_phys* &
       log10(h2otrip/t)+0.876793_kind_phys*(1._kind_phys-t/h2otrip)+ &
       log10(6.1071_kind_phys))*100._kind_phys
...
elemental function MurphyKoop_svp_water(t) result(es)
  ...
  es = exp(54.842763_kind_phys - (6763.22_kind_phys / t) - (4.210_kind_phys * log(t)) + &
       (0.000367_kind_phys * t) + (tanh(0.0415_kind_phys * (t - 218.8_kind_phys)) * &
       (53.878_kind_phys - (1331.22_kind_phys / t) - (9.44523_kind_phys * log(t)) + &
       0.014025_kind_phys * t)))
...
elemental function MurphyKoop_svp_ice(t) result(es)
  ...
  ! (good down to 110 K)
  es = exp(9.550426_kind_phys - (5723.265_kind_phys / t) + (3.53068_kind_phys * log(t)) &
       - (0.00728332_kind_phys * t))
...
elemental function OldGoffGratch_svp_water(t) result(es)
  real(kind_phys), intent(in) :: t
  real(kind_phys) :: es
  real(kind_phys) :: ps, e1, e2, f1, f2, f3, f4, f5, f

  ps = 1013.246_kind_phys
  e1 = 11.344_kind_phys*(1.0_kind_phys - t/tboil)
  e2 = -3.49149_kind_phys*(tboil/t - 1.0_kind_phys)
  f1 = -7.90298_kind_phys*(tboil/t - 1.0_kind_phys)
  f2 = 5.02808_kind_phys*log10(tboil/t)
  f3 = -1.3816_kind_phys*(10.0_kind_phys**e1 - 1.0_kind_phys)/10000000.0_kind_phys
  f4 = 8.1328_kind_phys*(10.0_kind_phys**e2 - 1.0_kind_phys)/1000.0_kind_phys
  f5 = log10(ps)
  f  = f1 + f2 + f3 + f4 + f5

  es = (10.0_kind_phys**f)*100.0_kind_phys
...
elemental function OldGoffGratch_svp_ice(t) result(es)
  real(kind_phys), intent(in) :: t
  real(kind_phys) :: es
  real(kind_phys) :: term1, term2, term3

  term1 = 2.01889049_kind_phys/(tmelt/t)
  term2 = 3.56654_kind_phys*log(tmelt/t)
  term3 = 20.947031_kind_phys*(tmelt/t)

  es = 575.185606e10_kind_phys*exp(-(term1 + term2 + term3))
...
elemental function Bolton_svp_water(t) result(es)
  real(kind_phys),parameter :: c1 = 611.2_kind_phys
  real(kind_phys),parameter :: c2 = 17.67_kind_phys
  real(kind_phys),parameter :: c3 = 243.5_kind_phys
