...
use physcons,    only: epsilo=>con_eps, &
                       latvap=>con_hvap, &
                       latice=>con_hfus, &
                       rh2o=>con_rv,   &
                       cpair=>con_cp,  &
...
! Goff & Gratch scheme.
real(kind_phys), parameter :: tboil = 373.16_kind_phys
...
! extent of the table.
  real(kind_phys), public, parameter :: tmin = 127.16_kind_phys
  real(kind_phys), public, parameter :: tmax = 375.16_kind_phys

  real(kind_phys), parameter :: ttrice = 20.00_kind_phys  ! transition range from es over H2O to es over ice

  integer :: plenest                             ! length of estbl
  real(kind_phys), allocatable :: estbl(:)              ! table values of saturation vapor pressure

  real(kind_phys) :: omeps      ! 1.0_kind_phys - epsilo

  real(kind_phys) :: c3         ! parameter used by findsp
  ...
  real(kind_phys) :: pcf(5) = (/ &
       5.04469588506e-01_kind_phys, &
       -5.47288442819e+00_kind_phys, &
       -3.67471858735e-01_kind_phys, &
       -8.95963532403e-03_kind_phys, &
       -7.78053686625e-05_kind_phys /)

!   --- Degree 6 approximation ---
!  real(kind_phys) :: pcf(6) = (/ &
!       7.63285250063e-02, &
!       5.86048427932e+00, &
!       4.38660831780e-01, &
!       1.37898276415e-02, &
...
!  use error_messages, only: handle_errmsg
  ...
  omeps  = 1.0_kind_phys - epsilo

  ! Transition range method is only valid for transition temperatures at:
  ! -40 deg C < T < 0 deg C
!  call shr_assert_in_domain(ttrice, ge=0._kind_phys, le=40._kind_phys, varname="ttrice",&
!       msg="wv_sat_init: Invalid transition temperature range.")

! This parameter uses a hardcoded 287.04_kind_phys?
  c3 = 287.04_kind_phys*(7.5_kind_phys*log(10._kind_phys))/cpair

! Init "methods" module containing actual SVP formulae.

  call wv_sat_methods_init(kind_phys, tmelt, h2otrip, tboil, ttrice, &
       epsilo, errstring)
...
!  call handle_errmsg(errstring, subname="wv_sat_methods_init")
  ...
  do i = 1, plenest
    estbl(i) = svp_trans(tmin + real(i-1,kind_phys))
  end do
...
elemental function estblf(t) result(es)
  ...
  real(kind_phys) :: t_tmp     ! intermediate temperature for es look-up

  real(kind_phys) :: weight ! Weight for interpolation

  t_tmp = max(min(t,tmax)-tmin, 0._kind_phys)   ! Number of table entries above tmin
  i = int(t_tmp) + 1                     ! Corresponding index.
  weight = t_tmp - aint(t_tmp, kind_phys)       ! Fractional part of t_tmp (for interpolation).
  es = (1._kind_phys - weight)*estbl(i) + weight*estbl(i+1)
...
elemental function tq_enthalpy(t, q, hltalt) result(enthalpy)
  ...
  real(kind_phys), intent(in) :: t      ! Temperature
  real(kind_phys), intent(in) :: q      ! Specific humidity
  real(kind_phys), intent(in) :: hltalt ! Modified hlat for T derivatives

  real(kind_phys) :: enthalpy
...
elemental subroutine no_ip_hltalt(t, hltalt)
  ...
  real(kind_phys), intent(in) :: t        ! Temperature
  ! Outputs
  real(kind_phys), intent(out) :: hltalt  ! Appropriately modified hlat
  ...
  if (t >= tmelt) then
     hltalt = hltalt - 2369.0_kind_phys*(t-tmelt)
  end if
...
elemental subroutine calc_hltalt(t, hltalt, tterm)
  ...
  real(kind_phys), intent(out), optional :: tterm

  ! Local variables
  real(kind_phys) :: tc      ! Temperature in degrees C
  real(kind_phys) :: weight  ! Weight for es transition from water to ice
  ! Loop iterator
  integer :: i

  if (present(tterm)) tterm = 0.0_kind_phys
...
elemental subroutine deriv_outputs(t, p, es, qs, hltalt, tterm, &
  ...
  ! Inputs
  real(kind_phys), intent(in) :: t      ! Temperature
  real(kind_phys), intent(in) :: p      ! Pressure
  real(kind_phys), intent(in) :: es     ! Saturation vapor pressure
  real(kind_phys), intent(in) :: qs     ! Saturation specific humidity
  real(kind_phys), intent(in) :: hltalt ! Modified latent heat
  real(kind_phys), intent(in) :: tterm  ! Extra term for d(es)/dT in
                                 ! transition region.

  ! Outputs
  real(kind_phys), intent(out), optional :: gam      ! (hltalt/cpair)*(d(qs)/dt)
  real(kind_phys), intent(out), optional :: dqsdt    ! (d(qs)/dt)

  ! Local variables
  real(kind_phys) :: desdt        ! d(es)/dt
  real(kind_phys) :: dqsdt_loc    ! local copy of dqsdt

  if (qs == 1.0_kind_phys) then
     dqsdt_loc = 0._kind_phys
  else
...
elemental subroutine qsat(t, p, es, qs, gam, dqsdt, enthalpy)
  ...
  real(kind_phys), intent(in) :: p    ! Pressure
  ! Outputs
  real(kind_phys), intent(out) :: es  ! Saturation vapor pressure
  real(kind_phys), intent(out) :: qs  ! Saturation specific humidity

  real(kind_phys), intent(out), optional :: gam    ! (l/cpair)*(d(qs)/dt)
  real(kind_phys), intent(out), optional :: dqsdt  ! (d(qs)/dt)
  real(kind_phys), intent(out), optional :: enthalpy ! cpair*t + hltalt*q

  ! Local variables
  real(kind_phys) :: hltalt       ! Modified latent heat for T derivatives
  real(kind_phys) :: tterm        ! Account for d(es)/dT in transition region
...
elemental subroutine qsat_water(t, p, es, qs, gam, dqsdt, enthalpy)
  ...
  if (present(gam) .or. present(dqsdt) .or. present(enthalpy)) then
     ...
     call deriv_outputs(t, p, es, qs, hltalt, 0._kind_phys, &
          gam=gam, dqsdt=dqsdt)
...
!
  ...
  real(kind_phys) gam                  ! change in sat spec. hum. wrt temperature (times hltalt/cpair)
  real(kind_phys) dgdt                 ! work variable
  real(kind_phys) g                    ! work variable
  real(kind_phys) hltalt               ! lat. heat. of vap.
  real(kind_phys) qs                   ! spec. hum. of water vapor

! work variables
  real(kind_phys) t1, q1, dt, dq
  real(kind_phys) qvd
  real(kind_phys) r1b, c1, c2
  real(kind_phys), parameter :: dttol = 1.e-4_kind_phys ! the relative temp error tolerance required to quit the iteration
  real(kind_phys), parameter :: dqtol = 1.e-4_kind_phys ! the relative moisture error tolerance required to quit the iteration
  real(kind_phys) enin, enout
  ...
  if (p <= 5._kind_phys*es .or. qs <= 0._kind_phys .or. qs >= 0.5_kind_phys &
  ...
  do l = 1, iter
     ...
     dq = abs(q1 - qsp)/max(q1,1.e-12_kind_phys)
     qsp = q1
  ...
  if (abs((enin-enout)/(enin+enout)) > 1.e-4_kind_phys) status = 8

end subroutine findsp

end module wv_saturation2
